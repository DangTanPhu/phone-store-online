...
import {
  ...
  getAdminProducts,
  getSuppliers,
  confirmReceiptAndUpdateInventory,
} from "../services/api";
...
const PurchaseOrderManagement = () => {
  ...
  const handleUpdateOrder = async (orderId, status, actualQuantity = null) => {
    try {
      if (status === "received") {
        ...
        const currentOrder = purchaseOrders.find(
        ...
        if (parseInt(actualQuantity) > currentOrder.suggestedQuantity) {
          alert(
            `Số lượng thực nhận (${actualQuantity}) không thể lớn hơn số lượng đề xuất (${currentOrder.suggestedQuantity})`
          );
          return;
        }

        await confirmReceiptAndUpdateInventory(
          orderId,
      ...
      alert(
        `Đã xảy ra lỗi khi cập nhật đơn đặt hàng: ${
  ...
  const handleDownloadPDF = async (orderId, type) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/admin/purchase-orders/${orderId}/${endpoint}`,
        {
          method: "GET",
          headers: {
      ...
      a.download =
        type === "receipt"
          ? `receipt-confirmation-${orderId}.pdf`
          : `purchase-order-${orderId}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      alert(
        `${
          type === "receipt" ? "Phiếu nhập kho" : "Phiếu đặt hàng"
        } đã được tải xuống thành công`
      );
    } catch (error) {
  ...
  const handleCreateOrder = async (e) => {
    ...
    try {
      ...
      if (response && response.purchaseOrder && response.purchaseOrder._id) {
        alert(
          `Đơn đặt hàng đã được tạo thành công với ID: ${response.purchaseOrder._id}`
        );
      } else {
      ...
      alert(
        `Có lỗi xảy ra khi tạo đơn đặt hàng: ${
          error.response?.data?.message || error.message
        }`
      );
    }
  ...
  const handleActualQuantityChange = (orderId, quantity) => {
    ...
    if (currentOrder && parseInt(quantity) > currentOrder.suggestedQuantity) {
      alert(
        `Số lượng thực nhận không thể lớn hơn số lượng đề xuất (${currentOrder.suggestedQuantity})`
      );
      return;
    }
    setActualQuantities((prev) => ({
              ...
              <td>
                {order.status === "approved" ? (
                  <input
                    ...
                    onChange={(e) =>
                      handleActualQuantityChange(order._id, e.target.value)
                    }
                    min="1"
